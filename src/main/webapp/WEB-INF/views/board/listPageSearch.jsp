...
<body>
	...
	<h2>Board</h2>
	<span style=" padding-left: 12px; font-size: 0.9em; color: gray;"> 방명록입니다</span>
	<div class="div1">	
	<div class="divA">
	...
	<table class ="table table-hover">
		...
		<tbody>
			<c:forEach items="${list}" var="list">
				<tr>
					<td>${list.bno}</td>
					<td><a href="/portfolio/board/view?bno=${list.bno}">${list.title}</a></td>
...
<a class="white" href="/portfolio/board/write">글 작성</a>
<br>
</div>
<nav class="text-center">
	...
	<c:if test="${page.prev}">
			<li class="page-item">
			<a href="/portfolio/board/listPageSearch?num=${page.startPageNum - 1}" class="page-link">이전</a></li>	
		</c:if>

		<c:forEach begin="${page.startPageNum}" end="${page.endPageNum}" var="num">
			<li class="page-item"><c:if test="${select != num}">
					<a href="/portfolio/board/listPageSearch?num=${num}" class="page-link">${num}</a>
		...
		<c:if test="${page.next}">
			 <li class="page-item"><a href="/portfolio/board/listPageSearch?num=${page.endPageNum + 1}" class="page-link">다음</a>
			</li>
		</c:if>
		</ul>
</nav>
	...
	<script>
		document.getElementById("searchBtn").onclick = function() {
			...
			location.href = "/portfolio/board/listPageSearch?num=1" + "&searchType="
